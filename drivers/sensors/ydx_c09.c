/*
 * Minimal YDX-C09 trackpoint driver (HID over I2C)
 */
...
const pointing_device_driver_t ydx_c09_pointing_device_driver = {
    .init       = ydx_c09_device_init,
    .get_report = ydx_c09_get_report,
    .set_cpi    = ydx_c09_set_cpi,
    .get_cpi    = ydx_c09_get_cpi,
};
...
void ydx_c09_set_cpi(uint16_t cpi) {
    ...
    i2c_status_t s1 = i2c_write_register(YDX_C09_ADDRESS << 1, 0x00, updown, sizeof(updown), YDX_C09_TIMEOUT);
    i2c_status_t s2 = i2c_write_register(YDX_C09_ADDRESS << 1, 0x00, leftright, sizeof(leftright), YDX_C09_TIMEOUT);

    #ifdef POINTING_DEVICE_DEBUG
        printf("ydx_c09 set speed: %d %d\n", s1, s2);
    #endif
}
...
void ydx_c09_reset(void) {
    ...
    i2c_status_t s = i2c_write_register(YDX_C09_ADDRESS << 1, 0x00, reset_cmd, sizeof(reset_cmd), YDX_C09_TIMEOUT);
#ifdef POINTING_DEVICE_DEBUG
    printf("ydx_c09 reset status: %d\n", s);
#endif
}


__attribute__((weak)) void ydx_c09_device_init(void) {
    i2c_init();
    wait_ms(100);
    ydx_c09_reset();
    wait_ms(100);
...
i2c_status_t read_ydx_c09(ydx_c09_data_t *data) {
    ...
    i2c_status_t status = i2c_receive(YDX_C09_ADDRESS << 1, (uint8_t*)data, sizeof(*data), YDX_C09_TIMEOUT);;

#ifdef POINTING_DEVICE_DEBUG
    static uint16_t d_timer;
    if (timer_elapsed(d_timer) > 250) {
        printf("ydx_c09 READ i2c_status_t: %d B: %02x X: %d Y: %d \n", status, data->buttons, data->x, data->y);
        d_timer = timer_read();
    }
#endif
...
report_mouse_t ydx_c09_get_report(report_mouse_t mouse_report) {
    ...
    if (error_count < 10) {
        i2c_status_t status = read_ydx_c09(&d);
