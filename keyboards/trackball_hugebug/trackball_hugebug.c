...
#    ifndef TRACKBALL_HUGE_BUG_MINIMUM_DEFAULT_DPI
#        define TRACKBALL_HUGE_BUG_MINIMUM_DEFAULT_DPI 400
#    endif // TRACKBALL_HUGE_BUG_MINIMUM_DEFAULT_DPI

#    ifndef TRACKBALL_HUGE_BUG_DEFAULT_DPI_CONFIG_STEP
#        define TRACKBALL_HUGE_BUG_DEFAULT_DPI_CONFIG_STEP 200
#    endif // TRACKBALL_HUGE_BUG_DEFAULT_DPI_CONFIG_STEP

#    ifndef TRACKBALL_HUGE_BUG_MINIMUM_SNIPING_DPI
#        define TRACKBALL_HUGE_BUG_MINIMUM_SNIPING_DPI 200
#    endif // TRACKBALL_HUGE_BUG_MINIMUM_SNIPING_DPI

#    ifndef TRACKBALL_HUGE_BUG_SNIPING_DPI_CONFIG_STEP
#        define TRACKBALL_HUGE_BUG_SNIPING_DPI_CONFIG_STEP 200
#    endif // TRACKBALL_HUGE_BUG_SNIPING_DPI_CONFIG_STEP

// Fixed DPI for drag-scroll.
#    ifndef TRACKBALL_HUGE_BUG_DRAGSCROLL_DPI
#        define TRACKBALL_HUGE_BUG_DRAGSCROLL_DPI 200
#    endif // TRACKBALL_HUGE_BUG_DRAGSCROLL_DPI

#    ifndef TRACKBALL_HUGE_BUG_DRAGSCROLL_BUFFER_SIZE
#        define TRACKBALL_HUGE_BUG_DRAGSCROLL_BUFFER_SIZE 6
#    endif // !TRACKBALL_HUGE_BUG_DRAGSCROLL_BUFFER_SIZE
...
static uint16_t get_pointer_default_dpi(trackball_sym8_config_t* config) {
    return (uint16_t)config->pointer_default_dpi * TRACKBALL_HUGE_BUG_DEFAULT_DPI_CONFIG_STEP + TRACKBALL_HUGE_BUG_MINIMUM_DEFAULT_DPI;
}

/** \brief Return the current value of the pointer's sniper-mode DPI. */
static uint16_t get_pointer_sniping_dpi(trackball_sym8_config_t* config) {
    return (uint16_t)config->pointer_sniping_dpi * TRACKBALL_HUGE_BUG_SNIPING_DPI_CONFIG_STEP + TRACKBALL_HUGE_BUG_MINIMUM_SNIPING_DPI;
}

/** \brief Set the appropriate DPI for the input config. */
static void maybe_update_pointing_device_cpi(trackball_sym8_config_t* config) {
    if (config->is_dragscroll_enabled) {
        pointing_device_set_cpi(TRACKBALL_HUGE_BUG_DRAGSCROLL_DPI);
 ...
 * The increment/decrement steps are equal to TRACKBALL_HUGE_BUG_DEFAULT_DPI_CONFIG_STEP.
 */
 ...
 *
 * Increases the DPI value if `forward` is `true`, decreases it otherwise.
 * The increment/decrement steps are equal to TRACKBALL_HUGE_BUG_SNIPING_DPI_CONFIG_STEP.
 */
...
#    ifdef TRACKBALL_HUGE_BUG_DRAGSCROLL_REVERSE_X
        scroll_buffer_x -= mouse_report->x;
#    else
        scroll_buffer_x += mouse_report->x;
#    endif // TRACKBALL_HUGE_BUG_DRAGSCROLL_REVERSE_X
#    ifdef TRACKBALL_HUGE_BUG_DRAGSCROLL_REVERSE_Y
        scroll_buffer_y -= mouse_report->y;
#    else
        scroll_buffer_y += mouse_report->y;
#    endif // TRACKBALL_HUGE_BUG_DRAGSCROLL_REVERSE_Y
        mouse_report->x = 0;
        mouse_report->y = 0;
        if (abs(scroll_buffer_x) > TRACKBALL_HUGE_BUG_DRAGSCROLL_BUFFER_SIZE) {
            mouse_report->h = scroll_buffer_x > 0 ? 1 : -1;
            scroll_buffer_x = 0;
        }
        if (abs(scroll_buffer_y) > TRACKBALL_HUGE_BUG_DRAGSCROLL_BUFFER_SIZE) {
...
#    if defined(POINTING_DEVICE_ENABLE) && !defined(NO_TRACKBALL_HUGE_BUG_KEYCODES)
...
#    endif // POINTING_DEVICE_ENABLE && !NO_TRACKBALL_HUGE_BUG_KEYCODES
...
#    ifdef POINTING_DEVICE_ENABLE
#        ifndef NO_TRACKBALL_HUGE_BUG_KEYCODES
    switch (keycode) {
...
#        endif // !NO_TRACKBALL_HUGE_BUG_KEYCODES
